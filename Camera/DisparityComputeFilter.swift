//
//  DisparityComputeFilter.swift
//  Camera
//
//  Created by Martin Pristas on 19.4.18.
...
class DisparityComputeFilter: CIFilter {
    ...
    let disparityKernel : CIKernel? = {
        ...
        "                        { \n" +
        "                                vec3 pixel_left = sample(leftImage, samplerTransform(leftImage, d + vec2(x ,y))).rgb; \n" +
        "                                vec3 pixel_right = sample(rightImage, samplerTransform(rightImage, d + vec2(x - disp,y))).rgb; \n" +
